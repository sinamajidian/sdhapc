...
{
	if (argc<4){ // How to use this code :  ./exec_file frag_file output_hap number_haps
	...
	for(iter=0;iter<num_read;iter++){
		...
		if(que_read[iter]==0){
			...
			if(colct>1){
				...
				int previous_index=0; // starting of the new block
				for(int kk=0; kk<i; kk++ ){
					if (previous_index==0){
					fprintf(file_cc_pt,"{%d:%d",col[kk],val[kk]);  // the first allele needs open bracet {
					}
					else{
						if(col[kk]<=previous_index) //new fragment
							fprintf(file_cc_pt,"}\n{%d:%d",col[kk],val[kk]);
						else // continuing fragment
							fprintf(file_cc_pt,", %d:%d",col[kk],val[kk]);
					}
					previous_index=col[kk];
				}
				fprintf(file_cc_pt,"}\n");
				iter_cnt++;
				if(rd_cnt>1){
